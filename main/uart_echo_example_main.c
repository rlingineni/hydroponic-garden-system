...
static void get_water_level(void *arg)
{
    ...
    while(1){
        int water_level = gpio_get_level(5);
...
static void get_ph_value(void *arg)
{
    ...
    while (1)
    {
        ...
        int ph_value = adc1_get_raw(ADC1_CHANNEL_6);
...
void app_main(void)
{
    ...
    blink_led();
     ...
     // xTaskCreate(blink_task, "get_water_level", 1024, NULL, 5, NULL);
    // xTaskCreate(get_water_level, "get_water_level", 1024, NULL, 5, NULL);
    xTaskCreate(get_ph_value, "get_ph_value", 1024, NULL, 5, NULL); // Create the get_ph_value task
    vTaskSuspend(myTaskHandle);
}
